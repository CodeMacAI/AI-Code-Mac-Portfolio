...
def draw_bar_plot():
    ...
    monthly_avg['month_name'] = pd.Categorical(monthly_avg['month'], categories=range(1, 13), ordered=True)
    monthly_avg = monthly_avg.sort_values('month')

    # Draw bar plot
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.barplot(data=monthly_avg, x='year', y='value', hue='month_name', palette='tab10', ax=ax)
...
def draw_box_plot():
    ...
    # Month-wise Box Plot
    sns.boxplot(data=df_box, x='month_name', y='value', order=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], ax=ax2)
